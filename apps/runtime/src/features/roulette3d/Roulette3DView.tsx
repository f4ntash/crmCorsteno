...
export function Roulette3DView({ config, slug }: { config: Roulette3DConfig; slug: string }) {
  ...
  async function fallbackSpin() { if (waiting) return; audio.current.unlock(); analytics.current.track('roulette_spin_click', { experienceId: slug }); setWaiting(true); try { const spinResult = await publicExperiencesApi.spin(slug, participant.current); analytics.current.track('roulette_spin_started', { experienceId: slug, spinId: spinResult.spinId }); analytics.current.track('roulette_spin_completed', { experienceId: slug, spinId: spinResult.spinId }); setResult(spinResult); } catch (e) { displaySpinError(e); } finally { setWaiting(false); } }
  async function spin() { if (waiting || spinning || (arState !== null && arState !== 'placed')) return; audio.current.unlock(); analytics.current.track('roulette_spin_click', { experienceId: slug }); setWaiting(true); setError(''); setResult(null); engineRef.current?.clearCelebration(); try { const spinResult = await publicExperiencesApi.spin(slug, participant.current); analytics.current.track('roulette_spin_started', { experienceId: slug, spinId: spinResult.spinId }); pendingResult.current = spinResult; if (!engineRef.current?.spinTo(spinResult.segmentIndex)) throw new Error('No se pudo iniciar el giro.'); } catch (e) { pendingResult.current = null; setWaiting(false); displaySpinError(e); } }
  ...
  return <div className="roulette-stage"><div ref={containerRef} className="roulette-3d" role="img" aria-label="Ruleta de premios" />{arStatus === 'supported' && !arState && <button className="ar-cta" type="button" onClick={() => void enterAR()}>Ver en AR</button>}{arState && <div className="ar-overlay" role="status"><p>{arState === 'searching' ? 'Mové el teléfono para encontrar una superficie.' : arState === 'ready' ? 'Tocá para colocar la ruleta.' : 'Ruleta colocada. Tocá Girar o la pantalla para jugar.'}</p>{arState === 'ended' ? null : <button className="secondary-cta" type="button" onClick={() => void leaveAR()}>Salir de AR</button>}</div>}<button className="spin-cta" type="button" disabled={waiting || spinning || (arState !== null && arState !== 'placed')} onClick={() => void spin()}>{waiting ? 'Preparando…' : spinning ? 'Girando…' : 'Girar'}</button>{arError && <p className="roulette-error" role="alert">{arError}</p>}{error && <p className="roulette-error" role="alert">{error}</p>}{result && <RouletteResult result={result} config={config} onCta={ctaClick} />}</div>;
}
