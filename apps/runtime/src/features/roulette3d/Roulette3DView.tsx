...
export function Roulette3DView({ config, slug }: { config: Roulette3DConfig; slug: string }) {
  ...
  async function spin() { if (waiting || spinning || (arState !== null && arState !== 'placed')) return; audio.current.unlock(); analytics.current.track('roulette_spin_click', { experienceId: slug }); setWaiting(true); setError(''); setResult(null); engineRef.current?.clearCelebration(); try { const spinResult = await publicExperiencesApi.spin(slug); analytics.current.track('roulette_spin_started', { experienceId: slug, spinId: spinResult.spinId }); pendingResult.current = spinResult; if (!engineRef.current?.spinTo(spinResult.segmentIndex)) throw new Error('No se pudo iniciar el giro.'); } catch (e) { pendingResult.current = null; setWaiting(false); setError((e as Error).message); } }
