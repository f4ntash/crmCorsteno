...
export class Roulette3D {
  ...
  constructor(config: Roulette3DConfig) {
  ...
  private animate = (time = performance.now()) => { this.animationFrame = requestAnimationFrame(this.animate); const delta = Math.min(0.05, (time - this.lastTime) / 1000); this.lastTime = time; this.root.rotation.z += delta * 0.018; this.renderer.render(this.scene, this.camera); };
}
