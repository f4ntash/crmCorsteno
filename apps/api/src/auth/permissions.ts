...
export type Permission = 'organization.read' | 'organization.manage' | 'project.read' | 'project.manage' | 'analytics.read' | 'crm.read' | 'crm.manage' | 'claims.redeem';
export const CRM_PERMISSIONS: Permission[] = ['crm.read', 'crm.manage', 'claims.redeem'];
const rank: Record<Exclude<Role, 'operator'>, number> = { viewer: 1, member: 2, admin: 3, owner: 4 };
const minimum: Record<Exclude<Permission, 'claims.redeem'>, Exclude<Role, 'operator'>> = { 'organization.read': 'viewer', 'project.read': 'viewer', 'analytics.read': 'member', 'crm.read': 'member', 'organization.manage': 'admin', 'project.manage': 'admin', 'crm.manage': 'admin' };
