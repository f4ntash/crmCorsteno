...
export const leads = sqliteTable(
  'leads',
  {
    ...
    source: text('source').notNull(), sourceReference: text('source_reference'), dedupeKey: text('dedupe_key'), score: integer('score'), recommendedOffer: text('recommended_offer'), recommendedDemo: text('recommended_demo'), status: text('status').notNull().default('NEW'), notes: text('notes'),
    lastContactAt: integer('last_contact_at', { mode: 'timestamp_ms' }), nextActionAt: integer('next_action_at', { mode: 'timestamp_ms' }), ...timestamps,
  },
  (t) => [index('leads_org_updated').on(t.organizationId, t.updatedAt), index('leads_org_status').on(t.organizationId, t.status, t.updatedAt), index('leads_org_domain').on(t.organizationId, t.domain), uniqueIndex('leads_org_dedupe_key').on(t.organizationId, t.dedupeKey)],
);
