...
export const leads = sqliteTable(
  'leads',
  {
    id: id(), organizationId: text('organization_id').notNull().references(() => organizations.id),
    businessName: text('business_name').notNull(), category: text('category').notNull(), subcategory: text('subcategory'), description: text('description'),
    website: text('website').notNull(), domain: text('domain').notNull(), city: text('city').notNull(), provinceState: text('province_state').notNull(), country: text('country').notNull(), address: text('address'),
    googleMapsUrl: text('google_maps_url'), instagramUrl: text('instagram_url'), linkedinUrl: text('linkedin_url'), phone: text('phone'), contactName: text('contact_name'), contactRole: text('contact_role'), contactEmail: text('contact_email'),
    source: text('source').notNull(), sourceReference: text('source_reference'), dedupeKey: text('dedupe_key'), score: integer('score'), recommendedOffer: text('recommended_offer'), recommendedDemo: text('recommended_demo'), status: text('status').notNull().default('NEW'), notes: text('notes'), archivedAt: integer('archived_at', { mode: 'timestamp_ms' }),
    lastContactAt: integer('last_contact_at', { mode: 'timestamp_ms' }), nextActionAt: integer('next_action_at', { mode: 'timestamp_ms' }), ...timestamps,
  },
  (t) => [index('leads_org_updated').on(t.organizationId, t.updatedAt), index('leads_org_status').on(t.organizationId, t.status, t.updatedAt), index('leads_org_domain').on(t.organizationId, t.domain), index('leads_org_archived_updated').on(t.organizationId, t.archivedAt, t.updatedAt), uniqueIndex('leads_org_dedupe_key').on(t.organizationId, t.dedupeKey)],
);
export const leadJobs = sqliteTable('lead_jobs', { id: id(), organizationId: text('organization_id').notNull().references(() => organizations.id), type: text('type').notNull(), status: text('status').notNull().default('QUEUED'), progress: integer('progress').notNull().default(0), total: integer('total').notNull().default(0), processed: integer('processed').notNull().default(0), succeeded: integer('succeeded').notNull().default(0), failed: integer('failed').notNull().default(0), metadata: text('metadata', { mode: 'json' }), error: text('error'), createdAt: integer('created_at', { mode: 'timestamp_ms' }).notNull(), startedAt: integer('started_at', { mode: 'timestamp_ms' }), updatedAt: integer('updated_at', { mode: 'timestamp_ms' }).notNull(), finishedAt: integer('finished_at', { mode: 'timestamp_ms' }) }, (t) => [index('lead_jobs_org_created').on(t.organizationId, t.createdAt)]);
...
export const products = sqliteTable(
  ...
  (t) => [uniqueIndex('products_organization_key').on(t.organizationId, t.productKey), index('products_organization_status').on(t.organizationId, t.status, t.updatedAt, t.id)],
);
export const productImages = sqliteTable(
  ...
  (t) => [uniqueIndex('product_images_product_asset').on(t.productId, t.assetId), index('product_images_product').on(t.productId, t.organizationId, t.sortOrder, t.id)],
);
export const productPublishedImages = sqliteTable(
  'product_published_images',
  {
    id: id(),
    organizationId: text('organization_id').notNull().references(() => organizations.id),
    productId: text('product_id').notNull().references(() => products.id),
...
export const catalogExperienceProducts = sqliteTable(
  ...
  (t) => [uniqueIndex('catalog_experience_products_unique').on(t.experienceId, t.productId), index('catalog_experience_products_order').on(t.experienceId, t.organizationId, t.sortOrder, t.id)],
);
export const catalogPublishedExperienceProducts = sqliteTable(
  'catalog_published_experience_products',
  {
    id: id(),
    organizationId: text('organization_id').notNull().references(() => organizations.id),
    experienceId: text('experience_id').notNull(),
  ...
  (t) => [uniqueIndex('catalog_published_experience_products_unique').on(t.experienceId, t.productId), index('catalog_published_experience_products_order').on(t.experienceId, t.organizationId, t.sortOrder, t.id)],
);
