...
adminRoutes.post('/users', requireAuth, async c => {
  ...
  const userId = existing?.id ?? crypto.randomUUID(); const now = Date.now(); const passwordHash = await hashPassword(password);
  await c.env.DB.batch([
    existing ? c.env.DB.prepare("UPDATE users SET email=?,name=?,status='active',password_hash=?,platform_role='user',updated_at=? WHERE id=?").bind(email, name, passwordHash, now, userId) : c.env.DB.prepare("INSERT INTO users (id,email,email_normalized,name,status,password_hash,platform_role,created_at,updated_at) VALUES (?,?,?,?,'active',?,'user',?,?)").bind(userId, email, email, name, passwordHash, now, now),
