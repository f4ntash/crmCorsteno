...
leadRoutes.get('/', read, async (c) => {
  ...
  const [rows, total, counts, jobs] = await Promise.all([
    c.env.DB.prepare(`SELECT ${select} FROM leads WHERE ${where.join(' AND ')} ORDER BY updated_at DESC, id DESC LIMIT ? OFFSET ?`).bind(...values, limit, offset).all(),
    c.env.DB.prepare(`SELECT COUNT(*) total FROM leads WHERE ${where.join(' AND ')}`).bind(...values).first<{ total: number }>(),
    c.env.DB.prepare(`SELECT status,COUNT(*) count FROM leads WHERE organization_id=? GROUP BY status`).bind(org).all<{ status: string; count: number }>(),
