...
function fixture(initialDraft = '{"segments":[]}', initialStatus = 'draft', initialPublished: string | null = null) {
  ...
  } }; }, batch(statements: Array<{ __sql?: string; __args?: any[]; run: () => Promise<{ success: boolean; meta?: { changes?: number } }> }>) { for (const statement of statements) { if (statement.__sql?.includes('experience_spins')) { const args = statement.__args ?? []; spins.push({ id: args[0], experienceId: args[1], organizationId: args[2], applicationId: args[3], segmentId: args[4], segmentIndex: args[5], prizeId: args[6], outcomeType: statement.__sql.includes("'prize'") ? 'prize' : 'no_prize', createdAt: '2026-01-03' }); } if (statement.__sql?.includes('experience_prize_inventory_events')) { const args = statement.__args ?? []; inventoryEvents.push({ id: args[0], experienceId: args[1], prizeId: args[2], type: 'prize_delivered', quantity: 1, spinId: args[3] }); } } return Promise.all(statements.map((statement) => statement.run())); } };
