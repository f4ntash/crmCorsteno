...
type InventoryItem = { experience_id: string; prize_id: string; stock_mode: 'limited' | 'unlimited'; stock_available: number | null; delivered_count: number };
function fixture(initialDraft = '{"segments":[]}', initialStatus = 'draft', initialPublished: string | null = null, role = 'owner', initialAccessPeriods: AccessPeriod[] = [], platformRole = 'corsteno_admin', initialInventory: InventoryItem[] = [], startsAt: string | null = null, endsAt: string | null = null, initialType = 'roulette', hostedStatus: 'active' | 'inactive' | null = 'active') {
  const experiences: E[] = [
    { id: 'a', organization_id: 'org-a', name: 'A', slug: 'a', type: initialType, status: initialStatus, schema_version: 1, draft_config: initialDraft, published_config: initialPublished, starts_at: startsAt, ends_at: endsAt, created_at: '2026-01-01', updated_at: '2026-01-01' },
  ...
  const inventory: InventoryItem[] = [...initialInventory];
    ...
    return {
      ...
      async first<T>() {
        ...
        if (sql.includes('auth_sessions')) return { session_id: 's', id: 'u', email: 'u@x', name: 'U', platformRole, expires_at: Date.now() + 10000 } as T;
          ...
          if (sql.includes('FROM experience_channels') && sql.includes("c.type='hosted_runtime'")) return hostedStatus ? { id: 'hosted-a', status: hostedStatus } as T : null as T;
          if (sql.includes('FROM channels') && sql.includes("type='hosted_runtime'")) return hostedStatus ? { id: 'hosted-a', status: hostedStatus } as T : null as T;
...
describe('experiences tenant isolation and validation', () => {
  ...
  it('creates Roulette through the existing creation contract', async () => {
    const env = fixture();
    const response = await request('/experiences', env, { method: 'POST', body: JSON.stringify({ name: 'Ruleta', type: 'roulette' }) });
    expect(response.status).toBe(201);
    expect(await response.json()).toEqual(expect.objectContaining({ type: 'roulette' }));
    expect((env as any).__channelLinks).toHaveLength(0);
  });
  it('keeps delivery opt-in and links an explicitly hosted experience to the reusable channel', async () => {
    const env = fixture();
    const hosted = await request('/experiences', env, { method: 'POST', body: JSON.stringify({ name: 'Alojada', type: 'roulette', delivery: 'hosted' }) });
    expect(hosted.status).toBe(201);
    expect((env as any).__channelLinks).toHaveLength(1);
  });
  ...
  it('lets an organization admin reuse configuration while resetting dates, inventory, and history', async () => {
    const config = {
      schemaVersion: 1,
      backgroundColor: '#111111',
      branding: { logoUrl: '/assets/organizations/org-a/experiences/a/123e4567-e89b-12d3-a456-426614174000.png' },
      content: { title: 'Evento', intro: 'Instrucciones', spinButtonLabel: 'Girar', winMessage: 'Ganaste', noPrizeMessage: 'Sin premio' },
      effects: { sound: true, vibration: false, celebration: true },
      resultCta: { enabled: true, label: 'Ver más', url: 'https://example.com' },
      participation: { maxSpinsPerDevice: 2, maxSpinsPerSession: null, cooldownSeconds: 30 },
      prizes: [{ id: 'p1', name: 'Premio limitado', stockMode: 'limited' as const, initialStock: 100 }, { id: 'p2', name: 'Premio ilimitado', stockMode: 'unlimited' as const }],
      segments: sixSegments().map((segment, index) => ({ ...segment, prizeId: index === 0 ? 'p1' : 'p2' })),
    };
    const env = fixture(JSON.stringify(config), 'published', JSON.stringify(config), 'admin', [], 'user', [{ experience_id: 'a', prize_id: 'p1', stock_mode: 'limited', stock_available: 17, delivered_count: 83 }], '2024-01-01T00:00:00.000Z', '2024-02-01T00:00:00.000Z') as any;
    env.__spins.push({ id: 'old-spin', experienceId: 'a', organizationId: 'org-a', prizeId: 'p1', outcomeType: 'prize', createdAt: '2024-01-15' });
    env.__claims.push({ id: 'old-claim', experienceId: 'a', prizeId: 'p1', status: 'redeemed', code: 'OLD' });
    const response = await request('/experiences/a/clone', env, { method: 'POST' });
    expect(response.status).toBe(201);
    const cloned = await response.json() as any;
    expect(cloned).toMatchObject({ organizationId: 'org-a', name: 'A - Copia', status: 'draft', publishedConfig: null, startsAt: null, endsAt: null });
    expect(cloned.id).not.toBe('a');
    expect(cloned.slug).not.toBe('a');
    expect(cloned.draftConfig).toEqual(config);
    expect(await (await request(`/experiences/${cloned.id}/spins`, env)).json()).toMatchObject({ items: [] });
    expect(await (await request(`/experiences/${cloned.id}/claims`, env)).json()).toMatchObject({ items: [] });
    const sourceInventory = await (await request('/experiences/a/inventory', env)).json() as { items: Array<{ prizeId: string; stockAvailable: number | null; deliveredCount: number }> };
    expect(sourceInventory.items.find((item) => item.prizeId === 'p1')).toEqual(expect.objectContaining({ stockAvailable: 17, deliveredCount: 83 }));
    expect((await request(`/experiences/${cloned.id}/publish`, env, { method: 'POST' })).status).toBe(200);
    expect(await (await request(`/experiences/${cloned.id}/inventory`, env)).json()).toEqual({ items: [expect.objectContaining({ prizeId: 'p1', stockAvailable: 100, deliveredCount: 0 }), expect.objectContaining({ prizeId: 'p2', stockMode: 'unlimited', stockAvailable: null, deliveredCount: 0 })] });
  });
  ...
  it('does not clone a foreign experience or allow members to clone', async () => {
    ...
    expect((await request('/experiences/a/clone', fixture('{"segments":[]}', 'draft', null, 'operator'), { method: 'POST' })).status).toBe(403);
  });
...
describe('roulette draft_config validation', () => {
  ...
  it('accepts campaign branding and content while preserving legacy configs', () => { const base = { schemaVersion: 1, backgroundColor: '#111111', prizes: [{ id: 'prize-1', name: 'Remera' }], segments: six() }; expect(validDraftConfig(base)).toBe(true); expect(validDraftConfig({ ...base, branding: { logoUrl: '/assets/organizations/org-a/experiences/a/123e4567-e89b-12d3-a456-426614174000.png', backgroundImageUrl: '/assets/organizations/org-a/experiences/a/123e4567-e89b-12d3-a456-426614174000.svg' }, content: { title: 'Campaña', intro: 'Girá la ruleta', spinButtonLabel: 'Jugar ahora', winMessage: 'Ganaste', noPrizeMessage: 'Seguí participando' } })).toBe(true); });
  it('rejects unsafe campaign assets and overlong campaign copy', () => { const base = { schemaVersion: 1, backgroundColor: '#111111', prizes: [{ id: 'prize-1', name: 'Remera' }], segments: six() }; expect(validDraftConfig({ ...base, branding: { logoUrl: 'javascript:alert(1)' } })).toBe(false); expect(validDraftConfig({ ...base, content: { title: 'x'.repeat(121) } })).toBe(false); });
  ...
  it('ignores stale CTA fields when the result button is disabled', () => { const base = { schemaVersion: 1, backgroundColor: '#111111', prizes: [{ id: 'prize-1', name: 'Remera' }], segments: six() }; expect(validDraftConfig({ ...base, resultCta: { enabled: false, label: '', url: '' } })).toBe(true); });
});
...
describe('experience participation limits', () => {
  ...
  it('prioritizes participation limits over sold-out availability', async () => {
    const published = JSON.stringify({ schemaVersion: 1, backgroundColor: '#111111', prizes: [{ id: 'prize-1', name: 'Remera', stockMode: 'limited', initialStock: 1, redemption: { enabled: true } }], segments: sixSegments(), participation: { maxSpinsPerDevice: 1, maxSpinsPerSession: null, cooldownSeconds: 0 } });
    const env = fixture(published, 'published', published);
    expect((await request('/experiences/a/publish', env, { method: 'POST' })).status).toBe(200);
    expect((await spin(env)).status).toBe(200);
    expect((env as any).__spins).toHaveLength(1);
    expect((env as any).__claims).toHaveLength(1);
    expect((await request('/experiences/a/inventory', env)).status).toBe(200);
    const blocked = await spin(env);
    expect(blocked.status).toBe(429);
    expect(await json(blocked)).toEqual(expect.objectContaining({ error: 'participation_limit_reached', reason: 'device_limit' }));
    const beforeOtherDevice = await request('/experiences/a/inventory', env);
    expect(await beforeOtherDevice.json()).toEqual({ items: [expect.objectContaining({ stockAvailable: 0, deliveredCount: 1 })] });
    const soldOut = await spin(env, ids.otherDevice, ids.otherSession);
    expect(soldOut.status).toBe(200);
    expect(await json(soldOut)).toEqual({ active: false, reason: 'unavailable' });
    expect((env as any).__spins).toHaveLength(1);
    expect((env as any).__claims).toHaveLength(1);
    const inventory = await request('/experiences/a/inventory', env);
    expect(await inventory.json()).toEqual({ items: [expect.objectContaining({ stockAvailable: 0, deliveredCount: 1 })] });
  });
