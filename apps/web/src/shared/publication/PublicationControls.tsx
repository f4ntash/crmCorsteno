...
export function PublicationControls({
  ...
  return <section className="publication-control" aria-label="Publicación y disponibilidad">
    ...
    {canChangeAvailability && <details className="publication-availability"><summary>Editar disponibilidad</summary><p className="field-help">Dejá el inicio vacío para comenzar de inmediato y el fin vacío para no establecer vencimiento. Las fechas se editan en tu zona horaria local.</p><div className="publication-availability-form"><label>Inicio<input type="datetime-local" value={startInput} onChange={(event) => setStartInput(event.currentTarget.value)} onInput={(event) => setStartInput(event.currentTarget.value)} /></label><label>Fin<input type="datetime-local" value={endInput} onChange={(event) => setEndInput(event.currentTarget.value)} onInput={(event) => setEndInput(event.currentTarget.value)} /></label><div className="publication-availability-actions"><button type="button" className="secondary" disabled={actionBusy} onClick={() => { setStartInput(''); setEndInput(''); setAvailabilityError(''); }}>Limpiar fechas</button><button type="button" disabled={actionBusy} onClick={() => void saveAvailability()}>{availabilitySaving || localAction ? 'Guardando…' : 'Guardar disponibilidad'}</button></div></div></details>}
