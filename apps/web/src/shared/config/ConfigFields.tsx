  ...
  return <div className="config-fields">
    {fields.map((field) => {
      ...
      return <div className="config-field" key={field.key}>
        ...
        {field.type === 'url' && <input id={inputId} type="text" inputMode="url" value={fieldValue(value)} disabled={disabled} required={field.required} maxLength={field.maxLength} placeholder={field.placeholder} onChange={(event) => onChange(field.key, event.target.value)} />}
        {field.type === 'image' && (assetContext ? <AssetPicker org={assetContext.org} value={typeof value === 'string' ? value : null} onChange={(next) => onChange(field.key, next)} categories={field.categories} canUpload={assetContext.canUpload} disabled={disabled} label={field.placeholder ?? 'Seleccionar imagen'} /> : <p className="config-field-missing">Se necesita una organización para seleccionar imágenes.</p>)}
