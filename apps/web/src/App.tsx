...
function ExperienceQr({ slug }: { slug: string }) {
  ...
  const url = `${runtimeBaseUrl}/r/${encodeURIComponent(slug)}`;
...
function validDraft(d: Draft) {
  ...
  return /^#[0-9a-f]{6}$/i.test(d.backgroundColor)
    && d.prizes.length >= 1 && d.prizes.length <= 5
    && new Set(d.prizes.map((prize) => prize.id)).size === d.prizes.length
    && d.prizes.every((prize) => prize.name.trim())
    && d.segments.length >= 6 && d.segments.length <= 10
    && d.segments.every((segment) => /^#[0-9a-f]{6}$/i.test(segment.color) && (segment.prizeId === null || d.prizes.some((prize) => prize.id === segment.prizeId)))
    && validLimit(participation.maxSpinsPerDevice)
    && validLimit(participation.maxSpinsPerSession)
    && Number.isInteger(participation.cooldownSeconds) && participation.cooldownSeconds >= 0 && participation.cooldownSeconds <= 604800;
}
...
function RouletteEditor({ org, id }: { org: string; id: string }) {
  ...
  if (!draft) return <main className="page"><p className="error">{error}</p></main>;
  const dirty = JSON.stringify(draft) !== original;
  const valid = validDraft(draft);
  const resize = (count: number) => setDraft((d) => d ? resizeDraft(d, count) : d);
  const update = (index: number, key: 'prizeId' | 'color', value: string) => setDraft((d) => d && ({ ...d, segments: d.segments.map((s, i) => i === index ? { ...s, [key]: key === 'prizeId' && value === '' ? null : value } : s) }));
  const updatePrize = (index: number, prize: Prize) => setDraft((d) => d && ({ ...d, prizes: d.prizes.map((p, i) => i === index ? prize : p) }));
  const removePrize = (index: number) => setDraft((d) => { if (!d) return d; const removed = d.prizes[index]?.id; return { ...d, prizes: d.prizes.filter((_, i) => i !== index), segments: d.segments.map((s) => s.prizeId === removed ? { ...s, prizeId: null } : s) }; });
  void removePrize;
  async function save() { if (!valid || !dirty || saving) return; setSaving(true); setMessage(''); setError(''); try { await get(`/experiences/${id}`, org, { method: 'PATCH', body: JSON.stringify({ draft_config: draft }) }); setOriginal(JSON.stringify(draft)); setMessage('Borrador guardado.'); } catch (e) { setError((e as Error).message); } finally { setSaving(false); } }
  ...
  const updateParticipation = (key: keyof Participation, value: string) => setDraft({ ...draft, participation: { ...draft.participation, [key]: value === '' ? (key === 'cooldownSeconds' ? 0 : null) : Number(value) } });
  return <main className="page"><div className="page-heading"><div><p className="eyebrow">EXPERIENCE / CONFIGURACIÓN</p><h1>{item?.name}</h1></div><button className="secondary" onClick={() => n('/app/experiences')}>Volver</button></div><div className="editor-grid"><section className="card editor-panel"><h2>Configuración</h2><label>Fondo<input type="color" value={draft.backgroundColor} onChange={(e) => setDraft({ ...draft, backgroundColor: e.target.value })} /></label><h3>Premios</h3>{draft.prizes.map((p, i) => <PrizeEditor key={p.id} prize={p} uploading={false} onChange={(next) => updatePrize(i, next)} />)}{draft.prizes.length < 5 && <button type="button" className="secondary" onClick={() => setDraft({ ...draft, prizes: [...draft.prizes, { id: `prize-${crypto.randomUUID()}`, name: `Premio ${draft.prizes.length + 1}` }] })}>+ Agregar premio</button>}<h3>Participación</h3><label>Límite por dispositivo<input type="number" min="1" max="100" placeholder="Sin límite" value={draft.participation.maxSpinsPerDevice ?? ''} onChange={(e) => updateParticipation('maxSpinsPerDevice', e.target.value)} /></label><label>Límite por sesión<input type="number" min="1" max="100" placeholder="Sin límite" value={draft.participation.maxSpinsPerSession ?? ''} onChange={(e) => updateParticipation('maxSpinsPerSession', e.target.value)} /></label><label>Tiempo entre giros (segundos)<input type="number" min="0" max="604800" value={draft.participation.cooldownSeconds} onChange={(e) => updateParticipation('cooldownSeconds', e.target.value)} /></label><label>Segmentos<select value={draft.segments.length} onChange={(e) => resize(Number(e.target.value))}>{[6,7,8,9,10].map((x) => <option key={x}>{x}</option>)}</select></label>{draft.segments.map((s, i) => <div className="segment-editor" key={s.id}><strong>Segmento {i + 1}</strong><select value={s.prizeId ?? ''} onChange={(e) => update(i, 'prizeId', e.target.value)}><option value="">Sin premio</option>{draft.prizes.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select><input type="color" value={s.color} onChange={(e) => update(i, 'color', e.target.value)} /></div>)}<div className="save-row">{dirty && <span className="dirty">Cambios sin guardar</span>}{message && <span className="success">{message}</span>}{error && <span className="error">{error}</span>}<button disabled={!valid || !dirty || saving} onClick={save}>{saving ? 'Guardando…' : 'Guardar borrador'}</button></div></section><section className="card preview-panel"><h2>Preview</h2><RoulettePreview segments={previewSegments} backgroundColor={draft.backgroundColor} /></section></div></main>;
}
...
function PrizeEditor({ prize, uploading, onChange }: { prize: Prize; uploading: boolean; onChange: (prize: Prize) => void }) {
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      const response = await fetch(`${api}/experiences/${experienceId}/assets`, { method: 'POST', credentials: 'include', headers: { 'X-Organization-Id': organizationId }, body: form });
      if (!response.ok) throw new Error('No se pudo subir el ícono.');
    ...
    <main className="page">
      ...
      {!s ? (
        ...
        <>
          <div className="grid">
            {Object.entries(s.totals)
              .filter(([k]) => {
                ...
                return [
                  ...generic,
                  ...(type === 'webar' ? webar : type === 'game' ? game : type === 'roulette' ? ['rouletteSpinsStarted', 'rouletteSpinsCompleted', 'roulettePrizesWon', 'rouletteNoPrize'] : []),
                ].includes(k);
              })
              .map(([k, v]) => (
                <div className="card" key={k}>
                  <small>
                    {{
                      ...
                      rouletteSpinsStarted: 'Giros iniciados',
                      rouletteSpinsCompleted: 'Giros completados',
                      roulettePrizesWon: 'Premios ganados',
                      rouletteNoPrize: 'Sin premio',
                    }[k] ?? k}
                  </small>
                  <b>
                    {k === 'lastActivityAt'
            ...
            {(apps.find((a) => a.id === application)?.applicationType ??
              'generic') === 'roulette' && (
              <div className="card"><small>Conversión a giro</small><b>{formatPercentage(s.rates.rouletteConversion)}</b></div>
            )}
            {(apps.find((a) => a.id === application)?.applicationType ??
              'generic') === 'game' && (
              <>
          ...
          {(apps.find((a) => a.id === application)?.applicationType ?? 'generic') === 'roulette' && <Rank title="Premios obtenidos" items={prizes} />}
          {(apps.find((a) => a.id === application)?.applicationType ??
            'generic') === 'game' && (
            <div className="chart-grid">
    ...
    <div className="shell">
      <aside>
        ...
        <nav>
          ...
          {isPlatformCommercialAdmin(m.user.platformRole) && <Link to="/app/onboarding">Nuevo cliente</Link>}
      ...
      <section className="content">
        ...
        <Routes>
          ...
          <Route path="onboarding" element={isPlatformCommercialAdmin(m.user.platformRole) ? <ClientOnboardingPage /> : <Navigate to="/app" replace />} />
          <Route
            path="*"
            element={
              <main className="page">
