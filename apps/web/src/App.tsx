...
function RouletteEditor({ org, id }: { org: string; id: string }) { const n = useNavigate(); const [item, setItem] = useState<Experience>(); const [draft, setDraft] = useState<Draft>(); const [original, setOriginal] = useState(''); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [message, setMessage] = useState(''); const [error, setError] = useState(''); useEffect(() => { if (!org) return; get(`/experiences/${id}`, org).then((x: Experience & { draftConfig: unknown }) => { const d = normalizeDraft(x.draftConfig); setItem(x); setDraft(d); setOriginal(JSON.stringify(d)); }).catch(() => setError('No se pudo cargar la experiencia.')).finally(() => setLoading(false)); }, [org, id]); if (loading) return <main className="page"><p>Cargando configuración…</p></main>; if (!draft) return <main className="page"><p className="error">{error}</p></main>; const dirty = JSON.stringify(draft) !== original; const valid = validDraft(draft); const resize = (count: number) => setDraft((d) => d ? resizeDraft(d, count) : d); const update = (index: number, key: 'prizeId' | 'color', value: string) => setDraft((d) => d && ({ ...d, segments: d.segments.map((s, i) => i === index ? { ...s, [key]: key === 'prizeId' && value === '' ? null : value } : s) })); const updatePrize = (index: number, name: string) => setDraft((d) => d && ({ ...d, prizes: d.prizes.map((p, i) => i === index ? { ...p, name } : p) })); const removePrize = (index: number) => setDraft((d) => { if (!d) return d; const removed = d.prizes[index]?.id; return { ...d, prizes: d.prizes.filter((_, i) => i !== index), segments: d.segments.map((s) => s.prizeId === removed ? { ...s, prizeId: null } : s) }; }); async function save() { if (!valid || !dirty || saving) return; setSaving(true); setMessage(''); setError(''); try { await get(`/experiences/${id}`, org, { method: 'PATCH', body: JSON.stringify({ draft_config: draft }) }); setOriginal(JSON.stringify(draft)); setMessage('Borrador guardado.'); } catch (e) { setError((e as Error).message); } finally { setSaving(false); } } const previewSegments = draft.segments.map((s) => ({ ...s, label: draft.prizes.find((p) => p.id === s.prizeId)?.name ?? 'Sin premio' })); return <main className="page"><div className="page-heading"><div><p className="eyebrow">EXPERIENCE / CONFIGURACIÓN</p><h1>{item?.name}</h1></div><button className="secondary" onClick={() => n('/app/experiences')}>Volver</button></div><div className="editor-grid"><section className="card editor-panel"><h2>Configuración</h2><label>Fondo<input type="color" value={draft.backgroundColor} onChange={(e) => setDraft({ ...draft, backgroundColor: e.target.value })} /></label><h3>Premios</h3>{draft.prizes.map((p, i) => <div className="prize-editor" key={p.id}><input value={p.name} onChange={(e) => updatePrize(i, e.target.value)} /><button className="secondary" type="button" onClick={() => removePrize(i)}>Eliminar</button></div>)}{draft.prizes.length < 5 && <button type="button" className="secondary" onClick={() => setDraft({ ...draft, prizes: [...draft.prizes, { id: `prize-${crypto.randomUUID()}`, name: `Premio ${draft.prizes.length + 1}` }] })}>+ Agregar premio</button>}<label>Segmentos<select value={draft.segments.length} onChange={(e) => resize(Number(e.target.value))}>{[6,7,8,9,10].map((x) => <option key={x}>{x}</option>)}</select></label>{draft.segments.map((s, i) => <div className="segment-editor" key={s.id}><strong>Segmento {i + 1}</strong><select value={s.prizeId ?? ''} onChange={(e) => update(i, 'prizeId', e.target.value)}><option value="">Sin premio</option>{draft.prizes.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select><input type="color" value={s.color} onChange={(e) => update(i, 'color', e.target.value)} /></div>)}<div className="save-row">{dirty && <span className="dirty">Cambios sin guardar</span>}{message && <span className="success">{message}</span>}{error && <span className="error">{error}</span>}<button disabled={!valid || !dirty || saving} onClick={save}>{saving ? 'Guardando…' : 'Guardar borrador'}</button></div></section><section className="card preview-panel"><h2>Preview</h2><RoulettePreview segments={previewSegments} backgroundColor={draft.backgroundColor} /></section></div></main>; }
function Login() {
  const n = useNavigate();
