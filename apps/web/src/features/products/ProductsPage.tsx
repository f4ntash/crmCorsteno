...
import { Product3DPanel } from './Product3DPanel';
import { ProductSurfacePanel } from './ProductSurfacePanel';
import './products.css';

type Props = { org: string; canEdit: boolean; canManageAssets?: boolean; isPlatformOperator?: boolean };
type Form = Pick<OrganizationProduct, 'name' | 'description' | 'priceMinorUnits' | 'currency' | 'priceUnit' | 'metadata' | 'stock' | 'mainAssetUrl' | 'ctaLabel' | 'ctaUrl'>;
type Errors = Partial<Record<CatalogProductField, string>>;
const empty: Form = { name: '', description: '', priceMinorUnits: 0, currency: 'ARS', priceUnit: null, metadata: null, stock: 0, mainAssetUrl: null, ctaLabel: null, ctaUrl: null };

function productForm(product: OrganizationProduct): Form {
  return { name: product.name, description: product.description, priceMinorUnits: product.priceMinorUnits, currency: product.currency, priceUnit: product.priceUnit ?? null, metadata: product.metadata ?? null, stock: product.stock, mainAssetUrl: product.mainAssetUrl, ctaLabel: product.ctaLabel, ctaUrl: product.ctaUrl };
}
...
export function ProductsPage({ org, canEdit, canManageAssets = false, isPlatformOperator = false }: Props) {
  ...
  function open(product: OrganizationProduct | 'new') {
    setEditing(product); setForm(product === 'new' ? empty : productForm(product)); setPriceInput(product === 'new' ? '0' : (product.priceMinorUnits / 100).toFixed(2).replace('.', ',')); setStockInput(product === 'new' ? '0' : String(product.stock)); setMetadataInput(product === 'new' || !product.metadata ? '' : JSON.stringify(product.metadata, null, 2)); setErrors({}); setSubmitted(false); setMessage('');
  }
  function validate(next = form, nextPrice = priceInput, nextStock = stockInput, nextMetadata = metadataInput) {
    return catalogProductFieldErrors({ ...next, priceMinorUnits: parseMoneyToMinor(nextPrice) ?? nextPrice, stock: /^\d+$/.test(nextStock) ? Number(nextStock) : nextStock, metadata: parseMetadataInput(nextMetadata), visible: true });
  }
  ...
  async function save(event: FormEvent) {
    ...
    try {
      const value = { ...form, priceMinorUnits: price, stock: Number(stockInput), metadata: parseMetadataInput(metadataInput) as Form['metadata'] };
