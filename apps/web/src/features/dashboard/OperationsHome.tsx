...
export function OperationsHome({ org, organizationName, isPlatformAdmin = false, canViewWorkspace = true }: { org: string; organizationName?: string; isPlatformAdmin?: boolean; canViewWorkspace?: boolean }) {
  ...
  if (!loading && !error && experiences.length === 0) return <main className="page operations-home"><div className="empty"><h2>Este workspace todavía no tiene productos configurados.</h2></div></main>;
  return <main className="page operations-home">
    <div className="home-heading"><div><p className="eyebrow">{organizationName ?? (isPlatformAdmin ? 'CORSTENO / ADMINISTRACIÓN' : 'ESPACIO DE TRABAJO')}</p><h1>Centro de operaciones</h1><p className="page-description">Tus experiencias, actividad reciente y próximos pasos.</p></div>{experiences.length > 0 && <Link className="button button-secondary" to="/app/experiences">Ver experiencias</Link>}</div>
