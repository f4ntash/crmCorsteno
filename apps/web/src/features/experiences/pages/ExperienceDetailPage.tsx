...
import { CRM_PRODUCT_LABELS } from '@corsteno/types';
...
function experienceProductLabel(type: string) {
  return type in CRM_PRODUCT_LABELS ? CRM_PRODUCT_LABELS[type as keyof typeof CRM_PRODUCT_LABELS] : type;
}
...
export function ExperienceDetailPage({
  ...
  const hostedRuntimeAvailable = Boolean(Editor) && hostedChannel?.status === 'active' &&
  ...
  const managedExperience = Boolean(Editor);
  return <main className="page experience-workspace">
    ...
    <nav className="workspace-nav" aria-label="Secciones de experiencia"><a href="#overview">Resumen</a>{managedExperience && <a href="#configuration">Configuración</a>}{isRoulette && <a href="#results">Resultados</a>}</nav>
    ...
    <section id="overview" className="workspace-section"><div className="workspace-section-heading"><div><p className="eyebrow">RESUMEN</p><h2>Estado operativo</h2></div><span className={`status status-${experience.status}`}>{experience.status === 'published' ? 'Publicada' : 'Borrador'}</span></div><div className="workspace-overview-grid"><section className="card workspace-summary"><h3>Canales de publicación</h3><p>{managedExperience ? 'La experiencia puede publicarse en un sitio externo o alojarse opcionalmente por Corsteno.' : 'Este registro identifica una experiencia de producto administrada fuera del editor del CRM.'}</p>{channels.length ? <ul className="workspace-channel-list">{channels.map((channel) => <li key={channel.id}><div><strong>{channel.name}</strong><small>{channelTypeLabels[channel.type]} · {channel.status === 'active' ? 'Activo' : 'Inactivo'}</small></div><div>{channel.type === 'hosted_runtime' ? publicUrl ? <a className="workspace-link" href={publicUrl} target="_blank" rel="noreferrer">Abrir enlace público →</a> : <span className="field-help">{channel.status !== 'active' ? 'Canal inactivo.' : experience.effective_status !== 'active' ? 'Disponible al publicar y activar la experiencia.' : 'No disponible por el acceso actual.'}</span> : channel.url && channel.status === 'active' ? <a className="workspace-link" href={channel.url} target="_blank" rel="noreferrer" aria-label={`Abrir ${channel.name} en una pestaña nueva`}>{channel.url}</a> : <span>{channel.url ?? 'Sin dominio registrado'}</span>}<small>{channel.type === 'hosted_runtime' ? 'Entrega opcional alojada por Corsteno' : channel.status === 'active' && channel.url ? 'Sitio público disponible' : 'Sitio registrado'}</small></div>{channel.type === 'hosted_runtime' && publicUrl && !isRoulette && <ExperienceQrModal slug={experience.slug} label="Ver QR" />}{channel.type === 'hosted_runtime' && canManage && <button type="button" className="button button-quiet" onClick={() => void disconnectHosted()} disabled={channelSaving}>{channelSaving ? 'Guardando…' : 'Desconectar'}</button>}</li>)}</ul> : <p className="field-help">No hay canales conectados a esta experiencia.</p>}{managedExperience && canManage && !hostedChannel && (availableHostedChannel ? <button type="button" className="button button-secondary workspace-channel-action" onClick={() => void connectHosted()} disabled={channelSaving}>{channelSaving ? 'Conectando…' : 'Conectar alojamiento de Corsteno'}</button> : <Link className="workspace-link workspace-channel-action" to="/app/channels">Crear canal alojado en Sitios y canales →</Link>)}{managedExperience && !canManage && !hostedChannel && <p className="field-help">Esta experiencia no tiene un canal alojado conectado.</p>}</section>{managedExperience ? <PublicationControls status={experience.effective_status} accessStatus={experience.access_status} hasUnpublishedChanges={hasUnpublishedChanges} canPublish={canManage} onPublish={publish} publishing={publishing} readinessIssues={readinessIssues} startsAt={experience.startsAt} endsAt={experience.endsAt} canEditAvailability={canManage} availabilitySaving={availabilitySaving} onSaveAvailability={saveAvailability} showStatus={false} readOnly={!canManage} message={publicationMessage} error={publicationError} /> : <section className="card workspace-summary"><h3>{experienceProductLabel(experience.type)}</h3><p>No hay editor ni configuración de este producto dentro del CRM. El registro conserva su estado y su referencia pública.</p>{experience.applicationId && <Link className="workspace-link" to="/app/analytics">Ver analytics →</Link>}</section>}<AccessPeriodPanel experienceId={id} organizationId={org} canManage={canManageCommercial} /></div></section>
    {managedExperience && <section id="configuration" className="workspace-section"><div className="workspace-section-heading"><div><p className="eyebrow">CONFIGURACIÓN</p><h2>{isRoulette ? 'Diseño, premios y participación' : 'Contenido y productos'}</h2></div></div><Editor org={org} id={id} redemptionAvailable={redemptionAvailable} brandingAvailable={brandingAvailable} canEdit={canManage} canAdjustInventory={canManage} canManageAssets={permissions.includes('assets.manage')} onDirtyChange={setDirty} onUnpublishedChange={setCatalogDirty} publication={isRoulette ? { status: experience.effective_status, accessStatus: experience.access_status, hasUnpublishedChanges, readinessIssues, canPublish: canManage, publishing, message: publicationMessage, error: publicationError } : undefined} onPublish={isRoulette ? () => void publish() : undefined} onDraftSaved={(draft) => { setExperience((current) => current ? { ...current, draftConfig: draft } : current); setReadinessIssues([]); setPublicationError(''); }} /></section>}
