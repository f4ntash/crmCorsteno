...
import { publicationStatusExplanation, publicationStatusView } from '../../../shared/publication/status';

type InventoryItem = { stockMode: 'limited' | 'unlimited'; stockAvailable: number | null };
type Props = { id: string; org: string; slug: string; status: EffectiveExperienceStatus; accessStatus?: string; startsAt: string | null; endsAt: string | null; publicUrl: string | null; onTest: () => void };
...
export function RouletteOperationsOverview({ id, org, slug, status, accessStatus, startsAt, endsAt, publicUrl, onTest }: Props) {
  ...
  const statusView = publicationStatusView(status, accessStatus);
  const context = accessLabels[accessStatus ?? ''] ?? publicationStatusExplanation(status, startsAt, endsAt, accessStatus);
  return <section className="roulette-operations" aria-label="Resumen operativo"><div className="roulette-operations-heading"><div><p className="eyebrow">OPERACIÓN DE CAMPAÑA</p><h2>Resumen operativo</h2><p>{context}</p></div><span className={`status status-${statusView.status}`}>{statusView.label}</span></div><div className="roulette-operation-metrics"><Metric label="Giros completados" value={spinSummary?.completed ?? null} /><Metric label="Premios ganados" value={spinSummary?.prizesWon ?? null} /><Metric label="Claims generados" value={claims?.generated ?? null} /><Metric label="Claims canjeados" value={claims?.redeemed ?? null} /><Metric label="Pendientes de canje" value={claims?.pending ?? null} />{limited.length > 0 && <Metric label="Stock limitado" value={`${available} con stock · ${soldOut} agotado${soldOut === 1 ? '' : 's'}`} />}</div><div className="roulette-quick-actions" aria-label="Acciones rápidas"><button type="button" className="primary" onClick={onTest}>Probar experiencia</button>{publicUrl && <><a href={publicUrl} target="_blank" rel="noreferrer">Abrir experiencia</a><ExperienceQrModal slug={slug} label="Ver QR" /></>}<a href="#configuration">Configurar</a><a href="#inventory">Inventario</a><a href="#results">Resultados</a></div></section>;
}
