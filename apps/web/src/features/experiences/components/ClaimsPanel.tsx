...
export function ClaimsPanel({ experienceId, organizationId, canRedeem }: { experienceId: string; organizationId: string; canRedeem: boolean }) {
  ...
  return <section className="card claims-panel"><h2>Premios a canjear</h2><div className="claim-search"><input aria-label="Código de claim" value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} placeholder="7KQ9-W2MF" /><button type="button" onClick={() => void search()} disabled={loading || !code.trim()}>Buscar</button></div>{claim && <div className="claim-result"><p><strong>Premio:</strong> {claim.prizeName}</p><p><strong>Estado:</strong> {claim.status === 'active' ? 'Activo' : 'Canjeado'}</p><p><strong>Generado:</strong> {new Date(claim.createdAt).toLocaleString('es-AR')}</p>{canRedeem && <button type="button" disabled={loading || claim.status !== 'active'} onClick={() => void redeem()}>{claim.status === 'active' ? 'Canjear premio' : 'Ya canjeado'}</button>}</div>}{message && <p className="field-help">{message}</p>}</section>;
}
