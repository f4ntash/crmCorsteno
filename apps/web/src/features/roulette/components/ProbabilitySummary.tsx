...
export function ProbabilitySummary({ draft, inventory, valid }: { draft: RouletteConfig; inventory: Inventory[]; valid: boolean }) {
  if (!valid || draft.segments.length === 0 || draft.prizes.length === 0) return <section className="probability-summary"><h3>Probabilidades efectivas</h3><p className="field-help">Completá premios y segmentos válidos para calcular las probabilidades.</p></section>;
  ...
  return <section className="probability-summary"><h3>Probabilidades efectivas</h3>{calculation.outcomes.length === 0 ? <p className="field-help">No hay resultados utilizables con la configuración actual.</p> : <div className="probability-list">{calculation.outcomes.map((outcome) => { const name = outcome.prizeId === null ? 'Sin premio' : draft.prizes.find((prize) => prize.id === outcome.prizeId)?.name ?? 'Premio'; return <div className="probability-row" key={outcome.prizeId ?? 'no-prize'}><div className="probability-label"><span>{name}</span><span>{outcome.probability.toFixed(1)}% <small>~{Math.round(outcome.frequencyPer100)} cada 100</small></span></div><div className="probability-bar" aria-hidden="true"><span style={{ width: `${Math.min(100, Math.max(0, outcome.probability))}%` }} /></div></div>; })}</div>}{excluded.length > 0 && <p className="field-help">No participan: {excluded.join(' · ')}.</p>}</section>;
}
