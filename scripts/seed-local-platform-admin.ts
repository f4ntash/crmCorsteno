...
function assertLocalOnly() {
  ...
  if (environment === 'production' || cloudflareEnvironment === 'production' || process.argv.includes('--remote') || process.argv.some((arg) => arg.startsWith('--remote='))) throw new Error('Refusing to seed the local platform admin in a production or remote environment.');
}
